...
- Caro 제품 동선의 단일 기준은 Figma `Caro Design`이다.
- Figma와 앱 구현이 다르면 Figma 동선을 앱에 맞춰 축소하지 않고 앱 구현 갭으로 판정한다.
- 이 QA 페이지의 연결과 증빙은 현재 앱 동작이 아니라 Figma에 정의된 목표 동선을 기준으로 한다.
...
| Figma 기준 동선 | 현재 앱 구현 | 판정 |
|---|---|---|
| 신규 로그인 → 프로필 생성 → Home | 로그인 성공 시 바로 Home 이동 | 구현 갭 |
| 덱 카드 탭 → 카드 상세 → 수정/삭제 | 카드 상세 NavKey 없이 카드 수정으로 직접 이동 | 구현 갭 |
| 보호 화면에서 인증 세션 만료 → Login | Splash 이외의 세션 만료 처리가 TODO | 구현 갭 |
| 덱 삭제 확인 → 삭제 완료 → Home | 덱 삭제 액션이 바텀시트만 닫고 종료 | 구현 갭 |
| 전체 학습 시작 시 카드 셔플 | 저장된 카드 목록 순서를 그대로 사용 | 구현 갭 |

## 범위

이번 결과는 Figma 기준 프로토타입 연결과 정적 화면 증빙이다. 위 구현 갭 수정과 실제 앱 빌드에서의 터치·백스택·API 성공/실패 동작은 별도의 구현·에뮬레이터 QA 대상이다.
