...
class DeckDetailViewModelTest :
    FunSpec({
        ...
        test("ClickCard 는 카드 상세 이동 side effect를 방출한다") {
            runTest(dispatcher) {
                val viewModel = createViewModel(deck = createDeck())
                val cardId = 100L

                viewModel.sideEffect.test {
                    viewModel.intent(DeckDetailIntent.ClickCard(cardId = cardId))

                    awaitItem() shouldBe DeckDetailSideEffect.NavigateToCardDetail(cardId = cardId)
                }
            }
        }
    })

private fun createViewModel(deck: Deck): DeckDetailViewModel =
    DeckDetailViewModel(
        deck = deck,
        exceptionFilter = ExceptionFilter.None,
    )
